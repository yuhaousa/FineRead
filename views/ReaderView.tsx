...
const ReaderView: React.FC<ReaderViewProps> = ({ text, onClose }) => {
  ...
  // Personalized Path Logic: Generate questions focused on the student's weak area and tailored to text type/difficulty
  useEffect(() => {
      ...
      <div className="w-full md:w-[450px] bg-slate-50 flex flex-col h-full border-l border-slate-200 shadow-2xl z-[220]">
        <div className="flex border-b border-slate-200 bg-white">
          <button 
            ...
            className={`flex-1 py-5 text-sm font-bold transition-all relative ${currentStep === 0 ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}
          >
            专项思维训练
            {currentStep === 0 && <div className="absolute bottom-0 left-0 w-full h-1 bg-indigo-600"></div>}
          </button>
          <button 
            onClick={() => setCurrentStep(1)}
            className={`flex-1 py-5 text-sm font-bold transition-all relative ${currentStep === 1 ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}
          >
            AI 阅读伙伴
            {currentStep === 1 && <div className="absolute bottom-0 left-0 w-full h-1 bg-indigo-600"></div>}
          </button>
        </div>
            ...
            <div className="space-y-6 animate-fadeIn">
              {isLoadingQuestions ? (
                <div className="space-y-4 py-8">
                  ...
                  <p className="text-center text-slate-400 text-[10px] font-bold tracking-widest uppercase">AI 正在为你的能力画像生成专属练习...</p>
                </div>
              ) : (
                questions.map((q) => (
              ...
              <div className="flex-1 space-y-4 pb-20">
                {messages.length === 0 && (
                  <div className="text-center py-16 space-y-6">
                    ...
                    <div>
                      <h4 className="font-bold text-slate-800">我是你的精读伙伴 Buddy</h4>
                      <p className="text-slate-400 text-xs px-12 leading-relaxed mt-2">在这里，我们不寻求标准答案，只开启深度对话。关于这篇《{text.title}》，你有什么新奇的想法吗？</p>
                    </div>
                  </div>
                )}
                {messages.map((m, idx) => (
